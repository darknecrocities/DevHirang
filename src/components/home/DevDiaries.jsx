            ...
            <div className="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/10 before:to-transparent">
                <AnimatePresence>
                    {allLogs.slice(0, visibleCount).map((log, index) => (
                        ...
                        >
                            {/* Dot */}
                            <div className="flex items-center justify-center w-10 h-10 rounded-full border border-white/10 bg-background group-hover:bg-secondary group-hover:border-white transition-all absolute left-0 md:left-1/2 md:-translate-x-1/2 z-10">
                                <div className="w-2 h-2 rounded-full bg-secondary group-hover:bg-primary animate-pulse" />
                            </div>

                            {/* Content */}
                            <div className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] glassmorphism p-6 rounded-3xl border group-hover:shadow-[0_0_30px_rgba(255,255,255,0.05)] transition-all bg-gradient-to-br ${getAchievementColor(log.type)}`}>
                                ...
                                <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-xs font-bold text-white">{log.devName}</p>
                                            <p className="text-[10px] text-text-muted font-medium uppercase tracking-tighter">{log.devRole}</p>
                                        </div>
                                    </div>
